...
import { addWeeks, subWeeks, addMonths, subMonths, format, startOfWeek, endOfWeek, startOfMonth, endOfMonth, subDays, addDays } from "date-fns";
import AppLayout from "@/components/AppLayout";
import { useAuth } from "@/hooks/useAuth";
import { supabase } from "@/integrations/supabase/client";
import { Button } from "@/components/ui/button";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Plus, CalendarDays, CalendarRange, Loader2 } from "lucide-react";
...
const Calendar = () => {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      const { data: calendarEvents, error } = await query;
      if (error) throw error;

      // Also pull workout sessions, cardio logs as virtual events
      const allEvents: CalendarEvent[] = (calendarEvents || []) as CalendarEvent[];

      // Load workout sessions for the user
      if (role === "client") {
        ...
        sessions?.forEach((s: any) => {
          ...
          if (!allEvents.find((e) => e.linked_workout_id === s.workout_id && e.event_date === format(new Date(s.created_at), "yyyy-MM-dd"))) {
            allEvents.push({
              id: `ws-${s.id}`,
              title: s.workouts?.name || "Workout",
              event_type: "workout",
              event_date: format(new Date(s.created_at), "yyyy-MM-dd"),
              is_completed: !!s.completed_at,
              completed_at: s.completed_at,
              is_recurring: false,
              user_id: user.id,
              description: null,
              event_time: format(new Date(s.created_at), "HH:mm"),
              end_time: null,
              color: null,
              notes: null,
        ...
        cardioLogs?.forEach((c: any) => {
          allEvents.push({
            ...
            title: c.title,
            event_type: "cardio",
            event_date: c.logged_at,
            is_completed: c.completed,
            is_recurring: false,
            user_id: user.id,
            description: `${c.cardio_type} • ${c.duration_min || "—"} min`,
            event_time: null,
            end_time: null,
            color: null,
            notes: null,
  ...
  const handleDelete = async (event: CalendarEvent) => {
    if (event.id.startsWith("ws-") || event.id.startsWith("cl-")) {
      toast({ title: "Cannot delete synced events here" });
      return;
    }
                  ...
                  {events
                    ...
                    .map((e) => (
                      <button
                        ...
                        className="w-full text-left flex items-center gap-2 p-2 rounded hover:bg-secondary/50 transition-colors"
                      >
                        <div className={`h-2 w-2 rounded-full shrink-0 ${
                          e.event_type === "workout" ? "bg-blue-500" :
                          e.event_type === "cardio" ? "bg-green-500" :
                          e.event_type === "checkin" ? "bg-purple-500" :
                          e.event_type === "rest" ? "bg-muted-foreground" :
                          "bg-primary"
                        }`} />
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-medium truncate">{e.title}</p>
                          <p className="text-[10px] text-muted-foreground">
                            {format(new Date(e.event_date), "EEE, MMM d")}
