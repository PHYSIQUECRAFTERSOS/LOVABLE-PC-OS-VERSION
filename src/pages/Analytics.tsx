...
import { Activity, TrendingDown, TrendingUp, Minus, Flame, Target, Brain, RefreshCw } from "lucide-react";
...
const GoalSelector = ({ currentGoal, onSaved }: { currentGoal: string | null; onSaved: () => void }) => {
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from("client_goals").upsert(
      { client_id: user.id, goal, target_rate: goal === "maintain" ? 0 : 0.5 },
    ...
    <AppLayout>
      <div className="animate-fade-in space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold text-foreground">Adaptive Engine</h1>
            <p className="mt-1 text-sm text-muted-foreground">TDEE estimation & auto-adjustments</p>
          </div>
          <div className="flex items-center gap-2">
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground max-w-md mx-auto">
                Log at least 3 days of weight entries and food to unlock your adaptive TDEE engine. The more data you provide, the smarter it gets.
              </p>
            </CardContent>
          </Card>
        ) : (
            ...
            <Card className="border-primary/20 bg-primary/5">
              <CardContent className="p-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground mb-1">Weekly Coaching Insight</p>
                  ...
                  <div className="h-52">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={chartData}>
                        ...
                        <Tooltip
                          contentStyle={{ backgroundColor: "hsl(0 0% 10%)", border: "1px solid hsl(0 0% 16%)", borderRadius: 8, color: "hsl(45 10% 90%)" }}
                        />
