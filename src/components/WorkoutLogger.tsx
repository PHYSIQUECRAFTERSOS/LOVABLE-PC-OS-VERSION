...
const WorkoutLogger = ({ workoutId, workoutName, workoutInstructions, exercises: initialExercises, onComplete, resumeSessionId, calendarEventId }: WorkoutLoggerProps) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      // History (previous performance / PRs / all-time bests) uses a shared
      // helper so the same lookup can run on-demand when the client swaps or
      // adds an exercise mid-session.
      const history = await fetchExerciseHistory(
        user.id,
        initialExercises.map((e) => ({ id: e.id, name: e.name })),
      );
      setPersonalRecords(history.personalRecords);
      setAllTimeBests(history.allTimeBests);
      if (Object.keys(history.previousPerformance).length > 0) {
        setPreviousPerformance(history.previousPerformance);
      }
  ...
  /**
   * Fetch "last time" data for a single exercise brought in mid-session
   * (substitution or ad-hoc add) and merge it into existing state. Runs in the
   * background — failures leave the exercise with no history, never an error.
   */
  const hydrateExerciseHistory = useCallback(async (exercise: { id: string; name?: string | null }) => {
    if (!user) return;
    try {
      const history = await fetchExerciseHistory(user.id, [{ id: exercise.id, name: exercise.name }]);
      if (Object.keys(history.previousPerformance).length > 0) {
        setPreviousPerformance((prev) => ({ ...prev, ...history.previousPerformance }));
      }
      if (Object.keys(history.allTimeBests).length > 0) {
        setAllTimeBests((prev) => ({ ...prev, ...history.allTimeBests }));
      }
      if (history.personalRecords.length > 0) {
        setPersonalRecords((prev) => [
          ...prev.filter((p) => !history.personalRecords.some((n) => n.exercise_id === p.exercise_id)),
          ...history.personalRecords,
        ]);
      }
    } catch (err) {
      console.warn("[WorkoutLogger] history hydrate failed", err);
    }
  }, [user]);
