...
describe("dbg", () => {
  it("dumps", () => {
    ...
    for (const w of s.workouts) {
      console.log("##", w.day_name, "(" + w.exercises.length + ")");
      for (const e of w.exercises) console.log(" -", JSON.stringify(e.name), "| sets="+e.sets, "reps="+e.reps, "rest="+e.rest_seconds, "g="+(e.grouping_id||"-"));
    }
  });
});
