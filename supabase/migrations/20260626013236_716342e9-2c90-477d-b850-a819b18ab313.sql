...
DROP FUNCTION IF EXISTS public.save_meal_with_items(TEXT, TEXT, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, JSONB);
...
BEGIN
  ...
  INSERT INTO public.saved_meals (
    client_id, name, meal_type, calories, protein, carbs, fat, servings, note
  ) VALUES (
    v_client_id,
    trim(p_name),
    ...
    NULLIF(trim(COALESCE(p_meal_note, '')), '')
  )
  RETURNING id INTO v_meal_id;

  INSERT INTO public.saved_meal_items (
    saved_meal_id, food_item_id, food_name, quantity, serving_unit, serving_size_g,
    calories, protein, carbs, fat,
    calories_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g, note
  )
  SELECT
    v_meal_id,
    -- Resolve FK: only keep food_item_id if the row actually exists, else NULL
    (SELECT fi.id FROM public.food_items fi
       WHERE fi.id = NULLIF(item.food_item_id, '')::UUID LIMIT 1),
...
GRANT EXECUTE ON FUNCTION public.save_meal_with_items(TEXT, TEXT, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, JSONB, TEXT) TO authenticated;
